...
class App extends React.Component {
  ...
  async componentDidMount() {
    ...
    const {identity, space} = await this.getIdentity()
   ...
   */
  getIdentity = async (): Promise<{identity: Libp2pCryptoIdentity, space: any}> => {
  ...
  getBucketKey = async () => {
    ...
    // Store the bucket key in our 3box space
    await this.state.space.private.set("bucketkey", root.key);
